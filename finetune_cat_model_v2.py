...
# モデルとトークナイザーの設定
MODEL_NAME = "./models/finetune_cat_model"  # 既存のファインチューニング済みモデル
OUTPUT_DIR = "./models/finetune_cat_model_v2"    # 新しい出力ディレクトリ
...
def main():
    ...
    training_args = TrainingArguments(
        ...
        save_total_limit=3,            # 保存するモデルの数を増やす
        logging_dir="./logs",
        logging_steps=50,              # より頻繁にログを出力
        learning_rate=5e-5,            # 学習率を調整
        weight_decay=0.0,              # 重みの減衰を無効化（過学習を許容）
        fp16=torch.cuda.is_available(),  # 16ビット精度を使用（GPUがある場合）
